...
def NBI():
    ...
    file_path = os.path.join(current_dir, '..', 'Input_data', 'NBI_Сoordinates.xlsx')
    ...
    #Start_point_line_NBI
    X_start = df.iloc[4:, 10].tolist()  # 8 colum, from 6 row
    Y_start = df.iloc[4:, 11].tolist()  # 9 colum, from 6 row
    Z_start = df.iloc[4:, 12].tolist()  # 10 colum, from 6 row
    
    
    #Last_point_line_NBI
    X_last = df.iloc[4:, 13].tolist()  # 8 colum, from 6 row
    Y_last = df.iloc[4:, 14].tolist()  # 9 colum, from 6 row
    Z_last = df.iloc[4:, 15].tolist()  # 10 colum, from 6 row
    ...
    #float
    X_start = [float(value) for value in X_start]
    Y_start = [float(value) for value in Y_start]
    Z_start = [float(value) for value in Z_start]

    #float
    X_last = [float(value) for value in X_last]
    Y_last = [float(value) for value in Y_last]
    Z_last = [float(value) for value in Z_last]
    ...
    return NBI_X, NBI_Y, NBI_Z, NBI_uvec_X, NBI_uvec_Y, NBI_uvec_Z,  X_start, Y_start, Z_start, X_last, Y_last, Z_last


def Ports():
    current_dir = os.path.dirname(os.path.abspath(__file__))
    file_path = os.path.join(current_dir, '..', 'Input_data', 'Ports_Сoordinates.xlsx')
    df = pd.read_excel(file_path)
    # P_1
